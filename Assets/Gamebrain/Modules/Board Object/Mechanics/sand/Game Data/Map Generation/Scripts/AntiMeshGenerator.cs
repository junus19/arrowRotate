...
public class AntiMeshGenerator : MonoBehaviour
{
    ...
    public void GenerateAntiMesh(float[,] m, float squareSize)
    {
        int xx = m.GetLength(0);
        int yy = m.GetLength(1);
        for (int x = 0; x < xx; x++)
        {
            for (int y = 0; y < yy; y++)
            {
        ...
        {
            uvs[i] = new Vector2(vertices[i].x / (100 * squareSize), vertices[i].y / (200 * squareSize));
        }
        mesh.uv = uvs;
