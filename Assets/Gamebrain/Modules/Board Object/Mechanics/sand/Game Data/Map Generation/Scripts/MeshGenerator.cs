...
public class MeshGenerator : MonoBehaviour
{
    ...
    public AntiMeshGenerator antiMesh;
    ...
    public void GenerateMesh(float[,] map, float squareSize)
    {
        ...
        if (antiMesh != null)
        {
            antiMesh.GenerateAntiMesh(map, squareSize);
        }
        ...
        // Generate UVs
        // Generate UVs with random offset
        // float randomOffsetX = Random.Range(0f, 0.05f);
        // float randomOffsetY = Random.Range(0f, 0.01f);
        
        Vector2[] uvs = new Vector2[vertices.Count];
        for (int i = 0; i < vertices.Count; i++)
        {
            uvs[i] = new Vector2(
                (vertices[i].x / (100 * squareSize) + xOffset/100),
                (vertices[i].y / (200 * squareSize) + yOffset/100)
            );
        }
        mesh.uv = uvs;

        // change x y offset of the texture
