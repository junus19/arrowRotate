...
- **Görsel (yalnız XZ; `LockKeyView.cs`):** grup renkleri `LockKeyFx.GroupColor(group)` (açık mavi/yeşil/pembe/sarı/mor/şeftali; kilit+anahtar aynı). `LockGroupView` = kilitli taşlarda koyu lid + segment gizli + centroid'e en yakın hexta **Lock ikonu** (grup renginde, scale 0.9). `KeyCellView` = **KOYU hexagon lid** + üstünde grup renginde **Key ikonu**; `TriggerToLock`: (1) BOUNCE — hexagon+ikon `OutBack` ile hafif yukarı çıkıp scale ile şişer, (2) anahtar ikonu kilide yay çizerek uçar + hexagon söner, (3) `OpenLock(group)` → lid'ler kalkar, segmentler görünür (oklar aktif). `ShakeLock` = kilit titrer. İkonlar `Billboard`+`Sprites/Default` quad. Sprite'lar **Lock_1 / Key_1** (tek renk → grup rengiyle tint'lenir), `BoardView.LockSprite/KeySprite`, Game.unity'de bağlı.
